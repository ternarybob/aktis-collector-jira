# -----------------------------------------------------------------------
# Deployment Script for Aktis Collector Jira
# -----------------------------------------------------------------------
...
<#
.SYNOPSIS
    Deploy and manage Aktis Collector Jira

.DESCRIPTION
    This script helps deploy and manage the Aktis Collector Jira service
...
.PARAMETER ConfigPath
    Path to configuration file (defaults to deployments/<target>/aktis-collector-jira.toml)
...
$executablePath = Join-Path -Path $binDir -ChildPath "aktis-collector-jira.exe"

Write-ColorOutput "Aktis Collector Jira Deployment Script" "Cyan"
...
if (-not $ConfigPath) {
    switch ($Target) {
        "local" {
            $ConfigPath = Join-Path -Path $projectRoot -ChildPath "deployments\local\aktis-collector-jira.toml"
            if (-not (Test-Path $ConfigPath)) {
                $ConfigPath = Join-Path -Path $binDir -ChildPath "aktis-collector-jira.toml"
            }
        }
        "docker" {
            $ConfigPath = Join-Path -Path $projectRoot -ChildPath "deployments\docker\aktis-collector-jira.toml"
        }
        "production" {
            $ConfigPath = Join-Path -Path $projectRoot -ChildPath "deployments\aktis-collector-jira.toml"
        }
    }
}
...
if ($Status) {
    ...
    if ($Target -eq "docker") {
        ...
        try {
            $dockerStatus = docker ps --filter "name=aktis-collector-jira" --format "{{.Status}}"
            if ($dockerStatus) {
...
if ($Logs) {
    ...
    if ($Target -eq "docker") {
        ...
        $logPath = Join-Path -Path $projectRoot -ChildPath "logs\aktis-collector-jira.log"
